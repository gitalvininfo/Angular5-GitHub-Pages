...
<html lang="en">
							...
							<div class="panel panel-primary">
								<div class="panel-heading">
									...
									<div class="btn-group pull-right">
										<div class="pull-left">
											<button class="btn btn-default btn-md" data-toggle="modal" data-target="#new_quotation">Add Quotation</button>
										</div>
									</div>
								</div>
	...
	$conn = new mysqli("localhost", "root", "", "sales") or die(mysqli_error());
			$query = $conn->query("SELECT * from `temp_trans` where `user_id` = '$_GET[id]' && `status` = 'Pending'") or die(mysqli_error());
			$grand = 0;
			while($fetch = $query->fetch_array()){
				...
				$grand=$grand+$total;
												...
												<tr>
													...
													<td><center	>
														<a href="#updatequotation<?php echo $fetch['temp_trans_id'];?>" data-target="#updatequotation<?php echo $fetch['temp_trans_id'];?>" data-toggle="modal" class="btn btn-default btn-sm">Edit</a>
														<a href="#deletequotation<?php echo $fetch['temp_trans_id'];?>" data-target="#deletequotation<?php echo $fetch['temp_trans_id'];?>" data-toggle="modal" class = "btn btn-sm btn-danger">Delete</a>
														</center>
													</td>
												</tr>
												<?php
		...
		<div class="modal fade" id="new_quotation" tabindex="-1" role="dialog" aria-labelledby="smallModalHead" aria-hidden="true">
			<div class="modal-dialog modal-sm">
				<div class="modal-content">
					...
					<form role="form" class="form-horizontal" action="actions/quoteadd.php" method="post" onsubmit="return confirm('Add to Quotation?');">
						...
						<div class="modal-footer">
							<button type="submit" class="btn btn-default" name="add">Add to Quotation</button>
		...
		<?php
		$conn = new mysqli("localhost", "root", "", "sales") or die(mysqli_error());
		$query = $conn->query("SELECT * FROM `temp_trans`") or die(mysqli_error());
		while($fetch = $query->fetch_array()){
		?>
		<div id="updatequotation<?php echo $fetch['temp_trans_id'];?>" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="smallModalHead" aria-hidden="true">
			<div class="modal-dialog modal-sm">
				<div class="modal-content">
					<div class="modal-header">
						<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
						<h4 class="modal-title" id="smallModalHead">Update Quotation</h4>
					</div>
					<form role="form" class="form-horizontal" action="actions/updatequotation.php" method="post" onsubmit="return confirm('Are you sure you want to update this product?');">
						<div class="modal-body">
							<div class="row">
								<div class="panel-body">
									...
									<div class="form-group ">
										<div class="col-md-12 col-xs-12">
											<input type="hidden"  name="user_id" value="<?php echo $_GET['id']?>"class="form-control"/>
											<input type="hidden" name="temp_trans_id" value="<?php echo $fetch['temp_trans_id']?>">
		...
		<div id="deletequotation<?php echo $fetch['temp_trans_id'];?>" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="smallModalHead" aria-hidden="true">
			<div class="modal-dialog modal-sm">
				<div class="modal-content">
					<div class="modal-header">
						<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
					...
					<form role="form" class="form-horizontal" action="actions/deletequotation.php" method="post">
						<div class="modal-body">
							<div class="row">
								<div class="panel-body">
									<input type="hidden"  name="user_id" value="<?php echo $_GET['id']?>"class="form-control"/>
		...
		<script>
			function pQout() {
				myWindow = window.open("print/printquotation.php?user_id=<?php echo $user_id?>", "", "width=1000, height=650");
			}
		</script>
