									...
									<div class="panel-body">
										<table id="print" class="table datatable">
											...
											<tbody>
												<?php
	require 'config.php';
			$query = $conn->query("SELECT *, count(*) as total FROM `users`, `purchase` where users.user_id = purchase.user_id && `type` = 'Customer' && status = 'Pending' group by fullname") or die(mysqli_error());
			while($fetch = $query->fetch_array()){
												...
												<tr>
													...
													<td><center><a href="customeroverallpurchase.php?id=<?php echo $fetch['user_id']?>" class="btn btn-default btn-md">Purchase <span class="badge badge-danger"><?php echo $fetch['total']?></span></a></center></td>
												</tr>
												<?php
			}
			$conn->close();
